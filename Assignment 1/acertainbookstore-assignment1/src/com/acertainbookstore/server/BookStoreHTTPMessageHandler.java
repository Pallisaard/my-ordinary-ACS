...
/**
 * {@link BookStoreHTTPMessageHandler} implements the message handler class which is invoked to
 * handle messages received by the {@link BookStoreHTTPServerUtility}. It decodes the HTTP message
 * and invokes the {@link CertainBookStore} server API.
 *
 * @see AbstractHandler
...
public class BookStoreHTTPMessageHandler extends AbstractHandler {
    ...
    private void getStockBooksByISBN(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private void getEditorPicks(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        String numBooksString =
                URLDecoder.decode(
                        request.getParameter(BookStoreConstants.BOOK_NUM_PARAM), StandardCharsets.UTF_8);
        BookStoreResponse bookStoreResponse = new BookStoreResponse();
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private void getBooksInDemand(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        BookStoreResponse bookStoreResponse = new BookStoreResponse();
    ...
    private void buyBooks(HttpServletRequest request, HttpServletResponse response)
        ...
        Set<BookCopy> bookCopiesToBuy =
                (Set<BookCopy>) serializer.get().deserialize(serializedRequestContent);
        BookStoreResponse bookStoreResponse = new BookStoreResponse();
     ...
     */
    @SuppressWarnings("unchecked")
    private void updateEditorPicks(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        byte[] serializedRequestContent = getSerializedRequestContent(request);

        Set<BookEditorPick> mapEditorPicksValues =
                (Set<BookEditorPick>) serializer.get().deserialize(serializedRequestContent);
        BookStoreResponse bookStoreResponse = new BookStoreResponse();
    ...
    private void addBooks(HttpServletRequest request, HttpServletResponse response)
        ...
        Set<StockBook> newBooks =
                (Set<StockBook>) serializer.get().deserialize(serializedRequestContent);
        BookStoreResponse bookStoreResponse = new BookStoreResponse();
     ...
     */
    @SuppressWarnings("unchecked")
    private void removeBooks(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        byte[] serializedRequestContent = getSerializedRequestContent(request);
    ...
    @SuppressWarnings("unchecked")
    private void rateBooks(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        byte[] serializedRequestContent = getSerializedRequestContent(request);

        Set<BookRating> bookRatings =
                (Set<BookRating>) serializer.get().deserialize(serializedRequestContent);
        BookStoreResponse bookStoreResponse = new BookStoreResponse();
    ...
    private void getTopRatedBooks(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        String numBooksString =
                URLDecoder.decode(
                        request.getParameter(BookStoreConstants.BOOK_NUM_PARAM), StandardCharsets.UTF_8);
